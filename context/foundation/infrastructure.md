...
- **Preview deploys**: Workers generates per-branch and per-commit preview URLs on `*.workers.dev`
  automatically. They are **public by default** — anyone with the URL can reach them. Because this
  app serves client PII, previews must either be gated behind Cloudflare Access or pointed at a
  throwaway Supabase project holding no real data. Decide which before the first branch deploy.
...
| Public preview URLs expose real client PII                                           | Unknown unknowns | M          | H      | Before the first branch deploy: either put Cloudflare Access in front of preview URLs, or point non-production deploys at a separate Supabase project containing only synthetic data.                                                                                                                                     |
| Compatibility-date bump enables process v2 → all SSR routes return `[object Object]` | Devil's advocate | M          | H      | Add `disable_nodejs_process_v2` to `compatibility_flags` in `wrangler.jsonc` pre-emptively, and treat any `compatibility_date` change as requiring a full smoke run.                                                                                                                                                      |
...
| First deploy locks production to the name `10x-astro-starter`                        | Devil's advocate | H          | M      | Rename `wrangler.jsonc` `name` (and `package.json` `name`) to `rozdzielnica-pro` **before** the first `wrangler deploy`. After the first deploy, renaming creates a second Worker.                                                                                                                                        |
| `tech-stack.md` says `cloudflare-pages`; adapter v14 dropped Pages                   | Devil's advocate | H          | M      | Correct the frontmatter to `cloudflare-workers`. Never run `wrangler pages deploy` — it is not interchangeable with `wrangler deploy`.                                                                                                                                                                                    |
| Adopting a Cloudflare binding (KV/D1/R2/DO/`waitUntil`) destroys portability         | Unknown unknowns | M          | M      | Treat bindings as out of bounds: Supabase already provides Postgres, Auth and storage. Use HTTP `Cache-Control` rather than the Cache API; put background work in a Supabase Edge Function rather than `waitUntil`.                                                                                                       |
